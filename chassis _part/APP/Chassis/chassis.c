...
void Chassis_RCDataHandle(Chassis_State State)
{
  ...
  switch(State)
  {
    ...
    case chassis_contrl:
    {
      ...
      OmniWheel_Chassis->Handle_RC.Chassis_Y =  First_orderfilter(&OmniWheel_Chassis->Handle_RC.Filter_Chassis_Yaw,OmniWheel_Chassis->Handle_RC.Chassis_Yaw);
      
      OmniWheelSpeedSolution(chassis_x,chassis_y,chassis_yaw);
...
void Chassis_lock(void)
{
  //输出电流
  //考虑加个位置环锁死,后面再来
  OmniWheel_Chassis->Chassis.LB_Wheel.Motor_Data.speed_pid_out = Pid_Parameter(&OmniWheel_Chassis->Chassis.LB_Wheel.Pid_speed,
                                                                                0,
                                                                                OmniWheel_Chassis->Chassis.LB_Wheel.Motor_Data.Motor_Get.speed_get);
  OmniWheel_Chassis->Chassis.LF_Wheel.Motor_Data.speed_pid_out = Pid_Parameter(&OmniWheel_Chassis->Chassis.LF_Wheel.Pid_speed,
                                                                                0,
                                                                                OmniWheel_Chassis->Chassis.LF_Wheel.Motor_Data.Motor_Get.speed_get);
  OmniWheel_Chassis->Chassis.RB_Wheel.Motor_Data.speed_pid_out = Pid_Parameter(&OmniWheel_Chassis->Chassis.RB_Wheel.Pid_speed,
                                                                                0,
                                                                                OmniWheel_Chassis->Chassis.RB_Wheel.Motor_Data.Motor_Get.speed_get);
  OmniWheel_Chassis->Chassis.RF_Wheel.Motor_Data.speed_pid_out = Pid_Parameter(&OmniWheel_Chassis->Chassis.RF_Wheel.Pid_speed,
                                                                                0,
                                                                                OmniWheel_Chassis->Chassis.RF_Wheel.Motor_Data.Motor_Get.speed_get);                                                                
}

void ChassisControlStatePid(void)
{
